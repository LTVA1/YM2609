...
class OPNBase : public Timer
{
    ...
    protected:
        ...
        {
            uint32_t[] slottable=new uint32_t[4]{ 0, 2, 1, 3 };
            uint8_t[] sltable = new uint8_t[16]{
        ...
        void SetPrescaler(uint32_t p)
        {
            ...
            if (prescale != p)
            {
                ...
                uint32_t ratio = ((fmclock << fmgen.FM_RATIOBITS) + rate / 2) / rate;
        ...
        uint32_t[] lfotable = new uint32_t[8];
        uint8_t prescale;
...
class OPNABase : public OPNBase
{
    ...
    //	チャンネルマスクの設定
    //
    public void SetChannelMask(uint32_t mask)
    {
        for (int i = 0; i < 6; i++)
    ...
    private void MakeTable2()
    {
        if (!tablehasmade)
        {
            ...
            {
                tltable[i + fmgen.FM_TLPOS] = (int)((uint32_t)(65536.0 * Math.Pow(2.0, i * -16.0 / fmgen.FM_TLENTS))) - 1;
            }
    ...
    //	初期化
    //
    protected bool Init(uint32_t c, uint32_t r, bool f)
    {
        RebuildTimeTable();
    ...
    //	サンプリングレート変更
    //
    protected bool SetRate(uint32_t c, uint32_t r, bool f)
    {
    ...
    //	レジスタアレイにデータを設定
    //
    protected void SetReg(uint32_t addr, uint32_t data)
    {
        int c = (int)(addr & 3);
        uint32_t modified;
    ...
    //	ADPCM B
    //
    protected void SetADPCMBReg(uint32_t addr, uint32_t data)
    {
        switch (addr)
        {
    ...
    //	レジスタ取得
    //
    protected uint32_t GetReg(uint32_t addr)
    {
        if (addr < 0x10)
    ...
    //	ステータスフラグ設定
    //
    protected new void SetStatus(uint32_t bits)
    {
        if ((status & bits)==0)
        {
    ...
    protected new void ResetStatus(uint32_t bits)
    {
        status &= ~bits;
        //	LOG1("ResetStatus(%.2x)\n", bits);
        UpdateStatus();
    ...
    protected void LFO()
    {
        ...
        chip.SetPML((uint32_t)(pmtable[(lfocount >> (fmgen.FM_LFOCBITS + 1)) & 0xff]));
        chip.SetAML((uint32_t)(amtable[(lfocount >> (fmgen.FM_LFOCBITS + 1)) & 0xff]));
        lfocount += lfodcount;
    }
    ...
    //	ADPCM 合成
    //	
    protected void ADPCMBMix(int[] dest, uint32_t count)
    {
    ...
    //	ADPCM RAM への書込み操作
    //
    protected void WriteRAM(uint32_t data)
    {
    ...
    protected int DecodeADPCMBSample(uint32_t data)
    {
    ...
    // FM 音源関係
    protected uint8_t[] pan = new uint8_t[6];
    protected uint8_t[] fnum2 = new uint8_t[9];

    protected uint8_t reg22;
    protected uint32_t reg29;     // OPNA only?

    protected uint32_t stmask;
    protected uint32_t statusnext;

    protected uint32_t lfocount;
    protected uint32_t lfodcount;

    protected uint32_t[] fnum = new uint32_t[6];
    protected uint32_t[] fnum3 = new uint32_t[3];

    // ADPCM 関係
    protected uint8_t[] adpcmbuf;        // ADPCM RAM
    protected uint32_t adpcmmask;     // メモリアドレスに対するビットマスク
    protected uint32_t adpcmnotice;   // ADPCM 再生終了時にたつビット
    protected uint32_t startaddr;     // Start address
    protected uint32_t stopaddr;      // Stop address
    protected uint32_t memaddr;       // 再生中アドレス
    protected uint32_t limitaddr;     // Limit address/mask
    protected int adpcmlevel;     // ADPCM 音量
    protected int adpcmvolume;
    protected int adpcmvol;
    protected uint32_t deltan;            // ⊿N
    protected int adplc;          // 周波数変換用変数
    protected int adpld;          // 周波数変換用変数差分値
    protected uint32_t adplbase;      // adpld の元
    ...
    protected uint32_t adpcmreadbuf;  // ADPCM リード用バッファ
    protected bool adpcmplay;     // ADPCM 再生中
    protected int8_t granuality;
    protected bool adpcmmask_;

    protected uint8_t control1;     // ADPCM コントロールレジスタ１
    protected uint8_t control2;     // ADPCM コントロールレジスタ２
    protected uint8_t[] adpcmreg = new uint8_t[8];  // ADPCM レジスタの一部分
